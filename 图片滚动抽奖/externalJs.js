...
$(function () {
    //需要进行抽奖数据放在这 begin  
    var zjArr = ["./i/2.jpg", "./i/3.jpg", "./i/4.jpg", "./i/5.jpg", "./i/6.jpg", "./i/7.jpg", "./i/8.jpg","./i/9.jpg", "xxx", "xxx"];
    //需要进行抽奖数据放在这 end
    ...
    var jsonArr = [
         { 'title': '三等：15张电影票', 'count': '1','zone':[] },
         { 'title': '二等：2张百货购物卡', 'count': '1', 'zone': ["./i/10.jpg"] },
         { 'title': '一等', 'count': '1', 'zone': ['./i/11.jpg'] }
    ];
    var timer = 0;  //全局定時
    var txtIndex = 0;
    ...
    $('body').keyup(function (e) {      
        ...
        if (e.which == 17) {     //抽      
            ...
            if (txtIndex % 2 == 0) {              
                ...
                if (globalIndex == jsonArr.length - 2)
                {
                    if (jsonArr[globalIndex].zone.length > 0)
                    {
                        var $currentArr=jsonArr[globalIndex].zone;
                        var $currentTxt = $currentArr[0];
                        $currentArr.splice(0,1);
                        $('#zjTxt img:first').attr("src",$currentTxt);
                        var removeIndex = zjArr.indexOf('xxx');
                        zjArr.splice(removeIndex, 1);
                        return;
                    }
                }

                if (globalIndex == jsonArr.length - 1) {
                    if (jsonArr[globalIndex].zone.length > 0) {
                        var $currentArr = jsonArr[globalIndex].zone;
                        var $currentTxt = $currentArr[0];
                        $currentArr.splice(0, 1);
                ...
                if ((zjArr.length - 2) >= jsonArr[globalIndex].count) {
